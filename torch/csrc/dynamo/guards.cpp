...
class GuardManager;
typedef std::pair<std::unique_ptr<GuardAccessor>, std::unique_ptr<GuardManager>> ChildGuardType;
class GuardManager {
public:
  ...
  GuardManager* get_attr_guard_manager(py::str name) {
    ...
    for (const auto& child_guard : _child_guards) {
      const GuardAccessor* child_accessor = child_guard.first.get();

      // Find the child accessor matching the new GuardAccessorT
      auto maybe_attr_accessor = dynamic_cast<const GuardAccessorT*>(child_accessor);
      if (maybe_attr_accessor != nullptr) {
        if (maybe_attr_accessor->equals(name)) {
          const auto& child_mananger = child_guard.second;
    ...
    std::unique_ptr<GuardAccessorT> accessor = std::make_unique<GuardAccessorT>(name);
...
PyObject* torch_c_dynamo_guards_init() {
  ...
  py::class_<GuardManager, std::unique_ptr<GuardManager>>(py_m, "GuardManager")
    ...
    .def("__getattr__", &GuardManager::get_attr_guard_manager<AttrGuardAccessor>, py::return_value_policy::reference)
    .def("__getitem__", &GuardManager::get_attr_guard_manager<ItemGuardAccessor>, py::return_value_policy::reference);
  return m;
}
